...
let ReniecMapper = class ReniecMapper {
    ...
    async mapearDatos(datos, formExistente, tiposSexo, estadosCivil) {
        ...
        parcial.idEstadoCivil = this.mapearEstadoCivil(String(datos.estadoCivil), estadosCivil);
        if (datos.direccion)
            parcial.direccionDomicilio = String(datos.direccion);
        await this.asignarUbigeos(datos, parcial, extra);
        return { form: parcial, ...extra };
    }
    parsearNombres(datos, form, parcial) {
        parcial.apellidoPaterno = String(datos.apellidoPaterno || form.apellidoPaterno || '').toUpperCase();
        parcial.apellidoMaterno = String(datos.apellidoMaterno || form.apellidoMaterno || '').toUpperCase();
        parcial.primerNombre = String(datos.primerNombre || form.primerNombre || '').toUpperCase();
        parcial.segundoNombre = String(datos.segundoNombre || form.segundoNombre || '').toUpperCase();
        if (!parcial.primerNombre && datos.nombres) {
            const partes = String(datos.nombres || '')
                ...
                .filter(Boolean);
            if (partes.length > 0)
                parcial.primerNombre = partes[0];
            if (partes.length > 1)
                parcial.segundoNombre = partes[1];
            if (partes.length > 2)
                parcial.tercerNombre = partes.slice(2).join(' ');
        }
    ...
    async asignarUbigeos(datos, parcial, extra) {
        ...
        if (d.departamento)
            parcial.idDepartamentoDomicilio = d.departamento;
        if (d.provincia)
            parcial.idProvinciaDomicilio = d.provincia;
        if (d.distrito)
            parcial.idDistritoDomicilio = d.distrito;
        const n = await this.autocompletarUbigeo(datos.departamentoNacimiento, datos.provinciaNacimiento, datos.distritoNacimiento);
        if (n.departamento)
            extra.depNacimientoSel = n.departamento;
        if (n.provincia)
            extra.provNacimientoSel = n.provincia;
        if (n.distrito)
            parcial.idDistritoNacimiento = n.distrito;
    }
    async autocompletarUbigeo(departamento, provincia, distrito) {
        ...
        if (dis)
            resultado.distrito = String(dis.id);
        return resultado;
    }
