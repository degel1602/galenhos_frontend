...
export class MaestrosApiService {
  ...
  getEtnias(): Promise<ICatalogoDescripcion[]> {
    return this.apiClient.request<any[]>('/api/v1/etnias').then(res => (res || []).map(e => ({ id: e.codigo ?? e.id, descripcion: e.descripcion })));
  }
  ...
  getParametro(id: number | string): Promise<IFilaBackend | IFilaBackend[]> { return this.apiClient.request<IFilaBackend | IFilaBackend[]>(`/api/v1/parametros/${id}`); }
