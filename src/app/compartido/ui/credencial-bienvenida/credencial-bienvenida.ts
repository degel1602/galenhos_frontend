...
export class CredencialBienvenidaComponent implements AfterViewInit, OnDestroy {
  @ViewChild('contenedor') readonly contenedorRef!: ElementRef<HTMLDivElement>;

  private readonly authService = inject(AuthService);
  private readonly bienvenidaService = inject(BienvenidaService);
  private readonly ngZone = inject(NgZone);

  readonly nombreUsuario = this.authService.username() ?? 'Usuario';

  private renderer!: THREE.WebGLRenderer;
  private escena!: THREE.Scene;
  private camara!: THREE.PerspectiveCamera;

  private maillaCard!: THREE.Mesh;
  private maillaClip!: THREE.Mesh;
  private maillaCinta!: THREE.Mesh;

  private mundo?: RAPIER.World;
  private cuerpoCard?: RAPIER.RigidBody;
  private cuerposSegmento: RAPIER.RigidBody[] = [];

  private arrastrando = false;
  private idPuntero = -1;
  private readonly offsetArrastre = new THREE.Vector3();
  private readonly planoArrastre = new THREE.Plane(
    new THREE.Vector3(0, 0, 1),
    0,
  );
  private readonly rayCaster = new THREE.Raycaster();
  private readonly posicionRaton = new THREE.Vector2();
  private animacionId = 0;
  private readonly limpiadores: Array<() => void> = [];

  private readonly puntoAnclaje = new THREE.Vector3(0, ANCHOR_Y, 0);
  ...
  private configurarRenderer() {
    ...
    this.renderer.setPixelRatio(
      Math.min(window.devicePixelRatio, esMobil ? 1.5 : 2),
    );
  ...
  private fabricarTarjeta(): THREE.Mesh {
  ...
  private pintarFrente(): THREE.CanvasTexture {
    const lienzo = document.createElement('canvas');
    lienzo.width = 512;
    lienzo.height = 720;
    const ctx = lienzo.getContext('2d');
    if (!ctx) return new THREE.CanvasTexture(lienzo);
    ...
    // Cargo MÁS GRANDE
    const cargo =
      this.authService.getIdEmpleado() > 0 ? 'MÉDICO TITULAR' : 'ADMINISTRADOR';
    ...
    ctx.fillText(
      this.authService.getIdEmpleado().toString() || '001',
      160,
      610,
    );
  ...
  private pintarDorso(): THREE.CanvasTexture {
    const lienzo = document.createElement('canvas');
    lienzo.width = 512;
    lienzo.height = 720;
    const ctx = lienzo.getContext('2d');
  ...
  private fabricarClip(): THREE.Mesh {
  ...
  private fabricarCintaInicial(): THREE.Mesh {
    const puntos = [
  ...
  private reconstruirCinta(posCard: THREE.Vector3, rotCard: THREE.Quaternion) {
    const puntoColgamiento = new THREE.Vector3(
      0,
      CARD_HEIGHT / 2 + CLIP_HEIGHT * 0.5,
      0,
    )
      .applyQuaternion(rotCard)
      .add(posCard);

    const puntos: THREE.Vector3[] = [this.puntoAnclaje.clone()];
  ...
  private iniciarFisicas() {
    RAPIER.init().then(() => {
      this.mundo = new RAPIER.World({ x: 0, y: -22, z: 0 });

      const anclajeBody = this.mundo.createRigidBody(
      ...
      for (let i = 0; i < 2; i++) {
        ...
        const segBody = this.mundo.createRigidBody(
        ...
        this.mundo.createImpulseJoint(
          RAPIER.JointData.spherical(
            { x: 0, y: 0, z: 0 },
            { x: 0, y: SEGMENT_LENGTH, z: 0 },
          ),
          cuerpoPrevio,
          segBody,
          true,
      ...
      this.cuerpoCard = this.mundo.createRigidBody(
        RAPIER.RigidBodyDesc.dynamic()
          .setTranslation(0, cardCenterY, 0)
          .setLinearDamping(2.0)
          .setAngularDamping(3.5),
      );
      this.mundo.createCollider(
  ...
  private onPointerDown(evento: PointerEvent) {
    if (!this.cuerpoCard || !this.mundo) return;
  ...
  private onPointerMove(evento: PointerEvent) {
    if (!this.cuerpoCard) return;
    ...
    for (const seg of this.cuerposSegmento) seg.wakeUp();
  }

  private onPointerUp(evento: PointerEvent) {
    if (
      !this.arrastrando ||
  ...
  private actualizarCursorRaton(clienteX: number, clienteY: number) {
    this.posicionRaton.set(
      (clienteX / window.innerWidth) * 2 - 1,
      -(clienteY / window.innerHeight) * 2 + 1,
    );
  }

  private proyectarEnPlano(
    clienteX: number,
    clienteY: number,
  ): THREE.Vector3 | null {
