...
import { TriajeApiService, } from '../../../modulos/triaje/adaptadores/salida/http/triaje.api.service';
...
import { formVacio, normalizarNombre, sanitizar, } from './registro-paciente.interfaces';
let RegistroPacienteService = class RegistroPacienteService {
    ...
    provNacimiento = [];
    distritosNacimiento = [];
    comunidadesNacimiento = [];
    depProcedencia = [];
    provProcedencia = [];
    distritosProcedencia = [];
    comunidadesProcedencia = [];
    depNacimientoSel = '';
    provNacimientoSel = '';
    depProcedenciaSel = '';
    provProcedenciaSel = '';
    ...
    async cargarCatalogos() {
        ...
        try {
            ...
            const asArray = (val) => Array.isArray(val) && val.length ? val : [];
    ...
    async onCambioDepartamento(tipo) {
        if (tipo === 'domicilio') {
    ...
    async cargarPaciente(pacienteId) {
    ...
    async cargarUbigeoEdicion() {
        const derivar = (idDistrito) => {
    ...
    construirPayload(base) {
        const f = this.form;
        const name = (v) => normalizarNombre(v || '') || undefined;
        const san = (v) => sanitizar(v || '') || undefined;
        const p = {
            ...base,
    ...
    async guardarComoTriaje() {
        ...
        const payload = this.construirPayload({
            ...
            idEsAccidenteTransito: f.idEsAccidenteTransito !== ''
    ...
    async actualizarPaciente(pacienteId) {
        const f = this.form;
        ...
        const payload = {
            ...
            homeAddress: sanitizar(f.direccionDomicilio) ||
            ...
            ethnicityId: f.idEtnia !== '' && f.idEtnia !== undefined && f.idEtnia !== null
                ? String(f.idEtnia)
...
RegistroPacienteService = __decorate([
    Injectable()
], RegistroPacienteService);
export { RegistroPacienteService };
function texto(v) {
