...
let FichaAdmisionComponent = class FichaAdmisionComponent {
    static { FichaAdmisionComponent_1 = this; }
    idCuentaAtencion;
        ...
        try {
            const instPromise = FichaAdmisionComponent_1.institucionCache
                ...
                : this.triajeApi.obtenerDatosInstitucion().then((r) => {
                    FichaAdmisionComponent_1.institucionCache =
                        r;
    ...
    generarHtmlFicha() {
        ...
        const casillaLetra = (letra) => `<span style="display:inline-block;width:15px;height:15px;border:1px solid #000;text-align:center;font-size:9px;line-height:15px;margin-right:2px;vertical-align:middle">${letra}</span>`;
      ...
      <td style="text-align:center;font-size:8.5px;font-weight:bold;padding:2px">Código CIE-10</td>
        ...
        const checklist = (items) => items
            .map((i) => `<span style="display:inline-flex;align-items:center;margin-right:14px;white-space:nowrap;font-size:9.5px">${casilla()}${i}</span>`)
            .join('');
        const filaChecklistAlineada = (items, colWidth = '25%') => `<tr>${items
            .map((i) => `<td style="width:${colWidth};padding:2px 4px 2px 0;white-space:nowrap;font-size:9.5px">${i ? casilla() + i : '&nbsp;'}</td>`)
            .join('')}</tr>`;
        const fechaHoraAtencion = ficha?.FechaHoraAtencion ??
...
__decorate([
    Input()
], FichaAdmisionComponent.prototype, "idCuentaAtencion", void 0);
__decorate([
    Output()
], FichaAdmisionComponent.prototype, "alCerrar", void 0);
__decorate([
    ViewChild('fichaFrame')
], FichaAdmisionComponent.prototype, "fichaFrame", void 0);
