...
function windowFirmaPeru() {
    return window;
}
function authHeaders(conexion, extra = {}) {
    const headers = {
...
function startSignatureGlobal() {
    const fn = windowFirmaPeru().startSignature;
    return typeof fn === 'function' ? fn : null;
}
async function apiRequest(conexion, path, init = {}) {
 ...
 */
async function asegurarJqFirmaPeru() {
...
export async function cargarLibreriaFirmaPeru() {
    if (startSignatureGlobal())
        return;
    await asegurarJqFirmaPeru();
    if (startSignatureGlobal())
        return;
    const existente = document.querySelector(`script[src="${FIRMA_PERU_LIB_URL}"]`);
    if (existente) {
    ...
    await new Promise((resolve, reject) => {
        s.onload = () => {
            if (startSignatureGlobal())
                ...
                reject(new Error('La librería firmaperu.min.js no expone startSignature.'));
        };
        s.onerror = () => reject(new Error('No se pudo cargar firmaperu.min.js desde el servidor de Firma Perú. Verifique su conexión.'));
        document.body.appendChild(s);
    });
}
/** Invoca startSignature y espera el documento/7z firmado (callbacks + polling). */
function esperarFirmaConFirmador(start, data, urlFirmado, conexion, batido) {
    return new Promise((resolve, reject) => {
        ...
        void (async () => {
            try {
                const firmado = await esperarDocumentoFirmado(data.documentNameUUID, urlFirmado, conexion, batido);
...
async function esperarDocumentoFirmado(uuid, urlFirmado, conexion, batido, opts = {}) {
    ...
    throw new Error('No se recibió el documento firmado del Firmador. Verifique que completó la firma en la aplicación Firma Perú.');
}
/** Devuelve el tipo MIME según el contenido (7z o PDF). */
function tipoContenido(bytes) {
 ...
 */
export async function iniciarFirmaLote(documentos, conexion, opts = {}, batido = {}) {
    await cargarLibreriaFirmaPeru();
    const form = new FormData();
    for (const d of documentos)
        form.append('document', d.blob, d.nombre);
    form.append('signatureFormat', 'PAdES');
    form.append('signatureLevel', 'B');
    if (opts.motivo)
        form.append('signatureReason', opts.motivo);
    if (opts.rol)
        form.append('role', opts.rol);
    if (opts.logoPngBase64)
        form.append('imageEstampado', opts.logoPngBase64);
    ...
    const env = await apiRequest(conexion, '/api/v1/firmaperu/lote', {
        method: 'POST',
    ...
    return esperarFirmaConFirmador(start, data, `/api/v1/firmaperu/documentos/${data.documentNameUUID}/lote/firmado`, conexion, batido);
}
