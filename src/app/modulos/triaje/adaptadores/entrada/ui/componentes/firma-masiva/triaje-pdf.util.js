...
export function formatFechaPdf(iso) {
    if (!iso)
        return '';
 ...
 */
export async function construirPdfTriaje(cabecera, institucion, idTriaje, usuario, fechaImp) {
    if (!cabecera)
        return null;
    const c = cabecera;
    ...
    const cw = w / nCols;
    let y = 15;
    function dibujarFila(celdas, h, valorSize = 7) {
        let x = ml;
