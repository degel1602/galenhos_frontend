import { iniciarFirmaLote } from './firma-peru.util';
import { construirPdfTriaje, } from './triaje-pdf.util';
...
async function obtenerReporte(conexion, idTriaje) {
 ...
 */
export async function generarPdfsTriajes(ids, institucion, conexion, usuario, onProgreso) {
    const generados = [];
    for (let i = 0; i < ids.length; i++) {
        const idTriaje = ids[i];
        if (!onProgreso(i + 1, ids.length, idTriaje))
            break;
        ...
        try {
            ...
            if (!reporte)
                throw new Error('No se encontró el reporte del triaje.');
            const doc = await construirPdfTriaje(reporte, institucion, idTriaje, usuario, formatFechaImp());
            if (!doc)
                throw new Error('No se pudo generar el PDF del triaje.');
 ...
 */
export async function firmarTriajesMasivamente(generados, institucion, conexion, usuario, onProgreso, batido = {}) {
    const conDoc = generados.filter((g) => g.doc);
    const sinDoc = generados.filter((g) => !g.doc);
    const resultados = sinDoc.map((g) => ({
        idTriaje: g.idTriaje,
        ok: false,
    ...
    if (!onProgreso(1, conDoc.length, primerTriaje))
        return resultados;
    console.log(`[FirmaPeru] masiva lote: firmando ${conDoc.length} documentos de una sola vez (PIN único)`);
    try {
        const documentos = conDoc.map((g) => ({
            blob: g.doc.output('blob'),
            nombre: `TRIAJE_${g.idTriaje}.pdf`,
        }));
        const firmado = await iniciarFirmaLote(documentos, conexion, {
        ...
        console.log(`[FirmaPeru] masiva lote uuid=${firmado.uuid} firmado 7z bytes=${firmado.firmado.size}`);
        for (const g of conDoc)
            resultados.push({ idTriaje: g.idTriaje, ok: true });
    }
    catch (e) {
        const msg = e instanceof Error ? e.message : 'Ocurrió un error durante la firma por lote de los triajes.';
