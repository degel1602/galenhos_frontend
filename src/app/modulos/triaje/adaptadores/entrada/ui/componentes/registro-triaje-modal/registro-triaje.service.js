...
let RegistroTriajeService = class RegistroTriajeService {
  ...
  parametroEsS(param) {
    ...
    const claves = ['valorTexto', 'ValorTexto', 'VALORTEXTO'];
  ...
  async consultarReniec() {
    ...
    try {
      ...
      if (resultado.datos) {
        const formComoPaciente = this.formulario;
  ...
  async obtenerUltimoTriajeId() {
    try {
      ...
      const ids = arregloItems.map((item) =>
...
RegistroTriajeService = __decorate([Injectable()], RegistroTriajeService);
