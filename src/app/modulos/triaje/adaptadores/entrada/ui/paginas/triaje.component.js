...
let TriajeComponent = class TriajeComponent {
    ...
    cargandoFicha = false;
    errorFicha = '';
    pacienteEvaluar = null;
    formEvaluacion = formEvaluacionVacio();
    ...
    async cargarCatalogos() {
        try {
            ...
            if (Array.isArray(serv))
                this.serviciosFiltro = serv;
            if (Array.isArray(serv))
                this.servicios = serv;
        }
        catch { }
    }
    abrirReporte(idTriaje) {
        if (idTriaje) {
    ...
    onFirmaMasivaCompletada(resumen) {
        this.mensajeExito =
            resumen.fallidos > 0
    ...
    abrirEvaluacion(paciente) {
        const ev = (paciente.evaluacion ?? undefined);
        const texto = (v) => {
            if (v === null || v === undefined)
                return '';
            if (typeof v === 'string')
    ...
    asignarValoresNumericos(payload, f) {
        const parse = (v) => Number(v.replace(',', '.'));
        const set = (key, v, minVal) => {
            const num = parse(v);
    ...
    construirPayloadEvaluacion(idTriaje, f, motivo, presionArterial) {
        const payload = {
            idTriaje,
            motivo,
            presionArterial,
