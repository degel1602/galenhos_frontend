...
export class MotivoService {
  ...
  async listarMotivos(idRegAtencion: number): Promise<MotivoAtencion[]> {
    try {
      const datos = await this.api.request<
        { motivo: string; fechaRegistro?: string }[]
      >(`/api/v1/evoluciones/${idRegAtencion}/motivos`, { method: 'GET' });
      if (!datos) return [];
      return datos.map((item, index) =>
        this.parsearMotivo(item, idRegAtencion, index),
      );
    } catch (error) {
