...
let EvolucionService = class EvolucionService {
    ...
    filteredPacientes = computed(() => {
        ...
        return this.pacientes().filter((p) => p.nombre.toLowerCase().includes(term) ||
    ...
    async cargarPacientes(resetPage = true) {
        ...
        try {
            ...
            if (this.fechaHasta())
                params.append('ffin', this.fechaHasta());
    ...
    async listarEvoluciones(idRegAtencion) {
        try {
            const data = await this.api.request(`/api/v1/evoluciones/paciente/${idRegAtencion}`, { method: 'GET' });
            return data ?? [];
        }
        catch (error) {
