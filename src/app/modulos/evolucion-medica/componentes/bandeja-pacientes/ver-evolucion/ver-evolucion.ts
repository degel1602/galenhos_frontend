...
export class VerEvolucionComponent {
  @Input({ required: true }) detalle!: any;
  ...
  get subjetivoMarcados(): string[] {
    ...
    const etiquetas: [string, boolean][] = [
      ['Dolor', !!s.dolor], ['Fiebre', !!s.fiebre], ['Tos', !!s.tos], ['Náuseas', !!s.nauseas],
      ['Vómitos', !!s.vomitos], ['Mareos', !!s.mareos], ['Disnea', !!s.disnea]
    ];
  ...
  get signosVitales(): { etiqueta: string; valor: string }[] {
    ...
    const items: [string, any][] = [
      ['P/A', sv.presionArterial], ['FC', sv.frecuenciaCardiaca], ['FR', sv.frecuenciaRespiratoria],
      ['T°', sv.temperatura], ['SpO₂', sv.saturacionOxigeno], ['Peso', sv.peso], ['Talla', sv.talla],
      ['IMC', sv.imc], ['Glucemia', sv.glucemia]
    ];
    return items.filter(([, v]) => v !== null && v !== undefined && v !== '' && v !== '—')
      .map(([e, v]) => ({ etiqueta: e, valor: String(v) }));
  }

  get examenFisico(): any[] {
  ...
  get diagnosticos(): any[] {
    ...
    return Array.isArray(arr) ? arr.filter(d => d?.descripcion || d?.cie10) : [];
  }
  ...
  get farmacologico(): any[] {
    const arr = this.detalle?.plan?.farmacologico;
    return Array.isArray(arr) ? arr.filter(m => m?.medicamento || m?.dosis) : [];
  }

  get procedimientosIndicados(): string[] {
    const p = this.detalle?.plan?.procedimientosIndicados ?? {};
    const items: [string, boolean][] = [
      ['Curaciones', !!p.curaciones], ['Suturas', !!p.suturas], ['Catéter', !!p.cateter],
      ['Intubación', !!p.intubacion]
    ];
    const activos = items.filter(([, v]) => v).map(([e]) => e);
    if (p.otro) activos.push(p.otro);
    return activos;
  }

  get solicitudExamenes(): { etiqueta: string; valor: string }[] {
    const e = this.detalle?.plan?.solicitudExamenes ?? {};
    const items: [string, any][] = [['Laboratorio', e.laboratorio], ['Imágenes', e.imagenes], ['Otros', e.otros]];
    return items.filter(([, v]) => v && String(v).trim())
  ...
  get interconsultasSolicitadas(): string[] {
    ...
    const items: [string, boolean][] = [
      ['Cardiología', !!i.cardiologia], ['Cirugía', !!i.cirugia], ['Nutrición', !!i.nutricion],
      ['Psicología', !!i.psicologia]
    ];
    const activos = items.filter(([, v]) => v).map(([e]) => e);
    if (i.otra) activos.push(i.otra);
    return activos;
  }
  ...
  get indicacionesGenerales(): { etiqueta: string; valor: string }[] {
    ...
    const items: [string, any][] = [
      ['Dieta', ig.dieta], ['Reposo', ig.reposo], ['Hidratación', ig.hidratacion],
      ['Oxígeno', ig.oxigeno], ['Restricciones', ig.restricciones]
    ];
    return items.filter(([, v]) => v && String(v).trim())
      .map(([e, v]) => ({ etiqueta: e, valor: String(v) }));
  }
