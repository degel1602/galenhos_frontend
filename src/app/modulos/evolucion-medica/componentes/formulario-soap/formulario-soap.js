...
let FormularioSoapComponent = class FormularioSoapComponent {
    static { FormularioSoapComponent_1 = this; }
    ...
    totalSintomasSeleccionados = computed(() => this.sintomasSeleccionados().size);
    ngOnInit() {
    ...
    irAnterior() {
        const idx = FormularioSoapComponent_1.ORDEN_PANELES.indexOf(this.activePanel());
        if (idx > 0)
    ...
    get esPrimerPanel() {
        return (FormularioSoapComponent_1.ORDEN_PANELES.indexOf(this.activePanel()) === 0);
    }
    get esUltimoPanel() {
        return (FormularioSoapComponent_1.ORDEN_PANELES.indexOf(this.activePanel()) ===
    ...
    async firmar() {
        ...
        const confirmado = await this.modalGlobal.confirmar('Se guardará la evolución completa con su firma. ¿Desea continuar?', 'Firmar evolución', 'Firmar');
        if (!confirmado)
            return;
        ...
        const dataB64 = btoa(encodeURIComponent(JSON.stringify(formData)).replace(/%([0-9A-F]{2})/g, (_match, p1) => String.fromCodePoint(Number(`0x${p1}`))));
