...
/** Lectura case-insensitive de una columna del SP (los SP retornan nombres de
 * columna tal cual; este helper tolera diferencias de mayúsculas). */
...
/** Traduce el patrón Blade `@if($campo == 'valor') X @endif`: devuelve "X" si coincide, si no vacío. */
function marcaSi(valorCampo: string, esperado: string): string {
  return valorCampo === esperado ? 'X' : ''
}

/** Genera un código de barras "decorativo" (equivalente visual al DNS1D::getBarcodeHTML de Milon). */
...
export function SisFuaReport({ idCuentaAtencion, onClose }: { idCuentaAtencion: number; onClose: () => void }) {
  const [fua, setFua] = useState<FuaDatos | null>(null)
  ...
  useEffect(() => {
    ...
    async function cargar() {
      try {
        await fetch('/api/v1/sis/fua/agregar', {
          method: 'POST',
        ...
        const [rImp, rInst] = await Promise.all([
          fetch(`/api/v1/sis/fua/imprimir?idCuentaAtencion=${idCuentaAtencion}`, { headers: authHeaders() }),
          fetch(`/api/v1/datos-institucion`, { headers: authHeaders() }),
        ])
        const envImp = await rImp.json().catch(() => null)
        const envInst = await rInst.json().catch(() => null)
        if (cancelled) return
        if (!rImp.ok || !envImp?.data) {
          setError(envImp?.error?.message ?? 'No se encontraron los datos del FUA.')
          return
        }
        setInstitucion((envInst?.data ?? null) as DatosInstitucion | null)
        const datos = envImp.data as Row

        const [rDias, rMed, rProc, rCons] = await Promise.all([
          fetch(`/api/v1/sis/diagnosticos?idCuentaAtencion=${idCuentaAtencion}`, { headers: authHeaders() }),
          fetch(`/api/v1/sis/medicamentos?idCuentaAtencion=${idCuentaAtencion}`, { headers: authHeaders() }),
          fetch(`/api/v1/sis/procedimientos?idCuentaAtencion=${idCuentaAtencion}`, { headers: authHeaders() }),
          fetch(`/api/v1/sis/consumo?idCuentaAtencion=${idCuentaAtencion}`, { headers: authHeaders() }),
        ])
        const parse = async (r: Response) => {
          const env = await r.json().catch(() => null)
          return Array.isArray(env?.data) ? (env.data as Row[]) : []
        }
        const [diag, med, proc, cons] = await Promise.all([parse(rDias), parse(rMed), parse(rProc), parse(rCons)])
        if (cancelled) return
        setFua({ datos, diagnosticos: diag, medicamentos: med, procedimientos: proc, consumo: cons })
      } catch {
        if (!cancelled) setError('No se pudo cargar el FUA.')
      } finally {
  ...
  function generarHtmlFua(): string {
    if (!fua) return ''
    const { datos, diagnosticos, medicamentos, procedimientos, consumo } = fua
  ...
  function imprimirFua() {
    imprimirHtml(generarHtmlFua())
  }

  return (
    <Modal title="Formato Único de Atención (FUA)" subtitle={`Cuenta N° ${idCuentaAtencion}`} onClose={onClose} width={880}>
      {cargando ? (
        ...
        <div>
          ...
          <div style={{ display: 'flex', justifyContent: 'flex-end', gap: 10, marginTop: 16, borderTop: '1px solid #e6eaf2', paddingTop: 16 }}>
            ...
            <button onClick={imprimirFua} className="gp-primary-btn" style={{ display: 'inline-flex', alignItems: 'center', gap: 8, padding: '10px 20px', background: '#263c7a', color: '#fff', border: 'none', borderRadius: 11, fontSize: 14, fontWeight: 600, cursor: 'pointer' }}>
